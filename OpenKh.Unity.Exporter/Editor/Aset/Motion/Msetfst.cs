...
namespace OpenKh.Unity.Exporter.Aset.Motion {
    ...
    public class FaceModifier {
        ...
        public FaceModifier(Stream si) {
            if (si.Length < 2)
                return;
            
            var br = new BinaryReader(si);
            var cnt1 = br.ReadByte(); // @0x00
            _ = br.ReadByte(); // @0x01

            br.ReadUInt16(); // @0x02
            for (var t1 = 0; t1 < cnt1; t1++) {
                var f1 = new Face();
                try {
                    f1.v0 = br.ReadInt16();
                    f1.v2 = br.ReadInt16();
                    ...
                    {
                        f1.v4 = br.ReadInt16();
                        f1.v6 = f1.v2 != -1 ? br.ReadInt16() : (short) 0;
                    }
                }
    ...
    public class Msetblk {
        ...
        public Msetblk(Stream si) {
            var p = new PoseTable(si);
            var baseoff = 0;
            var tbloff = p.tableOffset;
            ...
            var off1 = tbloff + p.vb4; int cnt1 = p.vb8;
            si.Position = off1;
            for (var a1 = 0; a1 < cnt1; a1++) {
                int c00 = br.ReadUInt16();
                int c02 = br.ReadUInt16();
                var c04 = br.ReadSingle();
                to.al1.Add(new T1(c00, c02, c04));
            }

            var off10 = tbloff + p.vd8;
            si.Position = off10;
            to.al10 = new float[cnt10];
            for (var a = 0; a < cnt10; a++) {
                to.al10[a] = br.ReadSingle();
            }

            var off11 = tbloff + p.vd4; int cnt11 = p.vb0;
            si.Position = off11;
            to.al11 = new float[cnt11];
            for (var a = 0; a < cnt11; a++) {
                to.al11[a] = br.ReadSingle();
            }

            var off12 = tbloff + p.vdc;
            si.Position = off12;
            to.al12 = new float[cnt12];
            for (var a = 0; a < cnt12; a++) {
                to.al12[a] = br.ReadSingle();
            }

            var off9 = tbloff + p.vd0;
            si.Position = off9;
            for (var a = 0; a < cnt9; a++) {
                int c00 = br.ReadUInt16();
                int c02 = br.ReadUInt16();
            ...
            var off2 = tbloff + p.vc0; int cnt2 = p.vc4;
            si.Position = off2;
            for (var a = 0; a < cnt2; a++) {
                int c00 = br.ReadByte();
                int c01 = br.ReadByte();
                int c02 = br.ReadByte();
                int c03 = br.ReadByte();
            ...
            for (var a = 0; a < cnt2; a++) {
                var o2 = to.al2[a];
                for (var b = 0; b < o2.c03; b++) {
                    var o9 = to.al9[o2.c04 + b];

                    var t9c00 = o9.c00; // t11_xxxx
                    var t9c02 = o9.c02; // t10_xxxx
                    var t9c04 = o9.c04; // t12_xxxx
                    var t9c06 = o9.c06; // t12_xxxx

                    o2.al9f.Add(new T9f(o2.c04 + b, to.al11[t9c00 >> 2], to.al10[t9c02], to.al12[t9c04], to.al12[t9c06]));
                }
            }

            var off2x = tbloff + p.vc8; int cnt2x = p.vcc;
            si.Position = off2x;
            for (var a = 0; a < cnt2x; a++) {
                int c00 = br.ReadByte();
                int c01 = br.ReadByte();
                int c02 = br.ReadByte();
                int c03 = br.ReadByte();
            ...
            var off3 = tbloff + p.ve0; int cnt3 = p.ve4;
            si.Position = off3;
            for (var a3 = 0; a3 < cnt3; a3++) {
                int c00 = br.ReadByte();
                int c01 = br.ReadByte();
                int c02 = br.ReadUInt16();
                int c04 = br.ReadUInt16();
                int c06 = br.ReadUInt16();
                var c08 = br.ReadUInt32();
                to.al3.Add(new T3(c00, c01, c02, c04, c06, c08));
            }

            var off4 = tbloff + p.vac;
            var cnt4 = p.va2;
            si.Position = off4;
            for (var a4 = 0; a4 < cnt4; a4++) {
                int c00 = br.ReadUInt16();
                int c02 = br.ReadUInt16();
            ...
            //hexVwer1.AddRangeMarked(baseoff + tableOffset + p.vb4, p.vb8 * 8, clr, clrb);
            //hexVwer1.AddRangeMarked(baseoff + tableOffset + p.vc0, p.vc4 * 6, clr, clrb);
            //hexVwer1.AddRangeMarked(baseoff + tableOffset + p.vc8, p.vcc * 6, clr, clrb);
            //hexVwer1.AddRangeMarked(baseoff + tableOffset + p.ve0, p.ve4 * 12, clr, clrb);
            //hexVwer1.AddRangeMarked(baseoff + tableOffset + p.vac, p.va2 * 4, clr, clrb);
            //hexVwer1.AddRangeMarked(baseoff + tableOffset + p.va8, (p.va2 - p.va0) * 64, clr, clrb);
            //hexVwer1.AddRangeMarked(baseoff + tableOffset + p.vf8, p.vfc * 12, clr, clrb);
            //hexVwer1.AddRangeMarked(baseoff + tableOffset + p.vf0, p.vf4 * 8, clr, clrb);
            //hexVwer1.AddRangeMarked(baseoff + tableOffset + p.vec, cntt8 * 48, clr, clrb);
            //hexVwer1.AddRangeMarked(baseoff + tableOffset + p.vd0, cnt9 * 8, clr, clrb);
            //hexVwer1.AddRangeMarked(baseoff + tableOffset + p.vd8, cnt10 * 4, clr, clrb);
            //hexVwer1.AddRangeMarked(baseoff + tableOffset + p.vd4, p.vb0 * 4, clr, clrb);
            //hexVwer1.AddRangeMarked(baseoff + tableOffset + p.vdc, cnt12 * 4, clr, clrb);
        }
    }
    internal class PoseTable {
        public int tableOffset = 0x90;
        ...
        public PoseTable(Stream si) {
            var br = new BinaryReader(si);
            var off = tableOffset - 0x90;
    ...
    public class Msetfst {
        public List<SingleMotion> al1 = new List<SingleMotion>();
        ...
        public Msetfst(Stream fs, string motionId) {
            ...
            foreach (var ent1 in ents1) {
                switch (ent1.k) {
                    case 17:
                        if (ent1.length != 0 && ent1.binaryData != null) {
                            var m1 = new SingleMotion();
                            var ents2 = ReadBar.Explode2(new MemoryStream(ent1.binaryData, false));
                            int k2 = 0;
                            foreach (var ent2 in ents2) {
                                switch (ent2.k) {
                                    case 9:
                                        m1.offset = (uint)ent1.offset + (uint)ent2.offset;
                                        m1.length = (uint)ent1.length;
                                        m1.id = ent1.id + "#" + ent2.id;
                                        m1.binaryData = ent2.binaryData;
                                        m1.k1 = k1;
                                        m1.isRaw = ent2.id.Equals("raw");
                                        break;
                                    case 16:
                                        if(ent2.binaryData != null)
                                            m1.faceModifier = new FaceModifier(new MemoryStream(ent2.binaryData, false));
                                        break;
                                }
                                k2++;
                            }
